...
export default function DebugSessionPage() {
  ...
  const hasAdminAccess = session?.user
    ? hasAnyRole(session.user as any, ["admin"])
    : false;
  const hasVendorAccess = session?.user
    ? hasAnyRole(session.user as any, ["vendor"])
    : false;
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="space-y-2">
              ...
              <p>
                ...
                <span
                  className={hasAdminAccess ? "text-green-600" : "text-red-600"}
                >
                  {hasAdminAccess ? "Yes" : "No"}
                </span>
              </p>
              <p>
                <strong>Has Vendor Access:</strong>{" "}
                <span
                  className={
                    hasVendorAccess ? "text-green-600" : "text-red-600"
                  }
                >
                  {hasVendorAccess ? "Yes" : "No"}
                </span>
              </p>
            </div>
          </div>
