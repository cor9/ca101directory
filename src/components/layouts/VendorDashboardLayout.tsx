    ...
    <div className="min-h-screen bg-background">
      <header className="bg-card border-b sticky top-0 z-50">
        <div className="container mx-auto flex h-16 items-center justify-between">
            <a href="/dashboard/vendor" className="font-bold text-lg text-paper">
                Vendor Dashboard
            </a>
            <nav className="flex items-center space-x-6 text-sm font-medium">
               {navItems.map((item) => (
                   <a key={item.href} href={item.href} className="text-paper transition-colors hover:text-paper">
                       {item.name}
                   </a>
               ))}
                 {/* This would be the standard sign-out link for next-auth */}
                <a href="/api/auth/signout" className="text-paper transition-colors hover:text-paper">Sign Out</a>
            </nav>
        </div>
      </header>
