...
export function SubmissionRow({ submission }: { submission: ItemInfo }) {
  const publishable =
    ...
    (submission.pricePlan === "pro" && submission.proPlanStatus === "success");
  ...
  return (
    <TableRow>
      ...
      <TableCell className="px-6 lg:px-0 w-[150px]">
        <span className="line-clamp-1 font-semibold">{submission.name}</span>
      </TableCell>
          ...
          <div className="hidden sm:block">
            <div className="flex items-center gap-2">
              ...
              <Button asChild variant="outline" size="sm">
                <Link href={`/edit/${submission._id}`}>Edit</Link>
              </Button>

              {/* show upgrade plan button if in free plan */}
              {submission.pricePlan === "free" ? (
                <Button asChild variant="outline" size="sm">
                  <Link href={`/plan/${submission._id}`}>Upgrade</Link>
                </Button>
              ) : null}
