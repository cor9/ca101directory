...
export const getPasswordResetTokenByEmail = async ( email: string ) => {
    try {
        ...
        const passResetToken = await sanityFetch<PasswordResetToken>({ query: passResetTokenQry, disableCache: true });
        if (SHOW_QUERY_LOGS) {
            console.log('getPasswordResetTokenByEmail, passResetToken:', passResetToken);
        }
...
export const getPasswordResetTokenByToken = async ( token: string ) => {
    try {
        ...
        const passResetToken = await sanityFetch<PasswordResetToken>({ query: passResetTokenQry, disableCache: true });
        if (SHOW_QUERY_LOGS) {
            console.log('getPasswordResetTokenByToken, passResetToken:', passResetToken);
        }
        return passResetToken;
    } catch (error) {
