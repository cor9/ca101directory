...
export default function ItemCardIH({ item }: ItemCardProps) {
                ...
                <div className="p-4">
                    <div className="flex justify-between items-center">
                        <h5 className="text-lg font-bold font-heading line-clamp-1">
                            {item.name?.find((kv) => kv._key === 'en')?.value || 'No Name'}
                        </h5>
                    </div>
                    {/* break-words */}
                    <p className="text-sm text-muted-foreground my-2 line-clamp-3">
                        {item.description?.find((kv) => kv._key === 'en')?.value || 'No Description'}
                    </p>
                </div>
            </Link>
        </div>
