...
export default async function CreateListingPage() {
  ...
  if (!user?.id) {
    redirect("/auth/login?callbackUrl=/dashboard/admin/create");
  }
      ...
      <div className="space-y-6">
        <div className="bg-card rounded-lg p-6 border">
          ...
          <p className="text-paper">
            Fill out the form below to add a new business to the directory. You
            can set the initial status and core details.
          </p>
        </div>

        <div className="bg-card rounded-lg p-6 border space-y-8">
          <div>
            <h2 className="text-xl font-semibold text-paper mb-3">
              Single Free Listing
            </h2>
            <p className="text-sm text-paper/80 mb-4">
              Use this form to create one Free plan listing. You can upgrade or
              edit later.
            </p>
            <AdminCreateForm />
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-paper/80 mb-4">
              Upload a CSV to create multiple Free plan listings. Accepted
              headers: name, description, website, email, phone, city, state,
              zip, region.
            </p>
